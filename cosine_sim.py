...
df_encoded = df_encoded.loc[df_clean.index]
...
# ---------------- RATING SLIDER ----------------
    ...
    if df_item.empty:
        ...
        ref_name = st.selectbox(
            "Choose a restaurant for recommendations",
            df_item["name"].unique()
        )

        ref_index = df_item[df_item["name"] == ref_name].index[0]

        similarity = cosine_similarity(
            df_encoded.loc[[ref_index]],
            df_encoded
        )[0]

        df_sim = df_clean.copy()
        df_sim["similarity"] = similarity
        df_sim = df_sim.drop(ref_index)

        df_sim = df_sim.sort_values(
            by=["similarity", "rating", "rating_count"],
            ascending=False
        ).head(10)
        ...
        for i, row in df_sim.iterrows():
            with cols[i % 2]:
                st.markdown(
                    f"""
                    <div style="background:#e8dcff; padding:15px; border-radius:12px;">
                        <h4>{row['name']}</h4>
                        <p>📍 {row['city']} | {row['Area']}</p>
                        <p>🍽 {row['Item']} | {row['cuisine']}</p>
